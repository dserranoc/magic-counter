...
export function Player ({ playerId }) {
  ...
  return (
    <div id={playerId} className={`${playerStyles.color} ${spanStyles && spanStyles[playersNumber] ? spanStyles[playersNumber] : ''} relative flex place-content-center items-center`}>
      <div className={`grid overflow-hidden ${showCommanderCounters ? 'grid-rows-2 grid-cols-2' : 'grid-rows-2'} ${rotationStyles && rotationStyles[playersNumber] ? rotationStyles[playersNumber] : ''}`}>
        <div className='flex items-center place-content-center'>
          <Counter minValue={MIN_VALUE} maxValue={MAX_VALUE} defaultValue={DEFAULT_LIVES} setValue={setLives} value={lives} areCommanderShown={showCommanderCounters} />
        </div>
        <div className='flex items-center place-content-center'>
          <Counter minValue={MIN_VALUE} maxValue={MAX_VALUE} defaultValue={DEFAULT_LIVES} setValue={setPoison} value={poison} style='bg-white/50 rounded-md' areCommanderShown={showCommanderCounters} />
        </div>
        {gamemode !== 'duel' && (
